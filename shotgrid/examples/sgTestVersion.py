...
def version(project, asset, task, basefolder):
...
# exit(1)
mov_path = r'C:\DATA\temp\sh_001.mov'
mov_path = r'V:/home/svilla/temp/sandbox/shot_1/Light/version_5/sh_001_v4.mov'
if False:
    print("Creating mp4...")
    # vcodec = "-vcodec libx264 -pix_fmt yuv420p -vf 'scale=trunc((a*oh)/2)*2:%{height}' -g 30 -vprofile high -bf 0 -crf 23"
    vcodec = "-vcodec libx264 -pix_fmt yuv420p -vf 'scale=iw/2:-1' -g 30 -vprofile high -bf 0 -crf 23"
    cmd = [ ffmpeg_exe, "-y", "-i", "\"%s\"" % mov_path,
               "-strict", "experimental",
               "-acodec", "aac", "-ab", "160k", "-ac", "2", "-vcodec",
               "libx264", "-pix_fmt", "yuv420p", "-vf",
               "\"scale=trunc((a*oh)/2)*2:720\"", "-g", "30", "-b:v", "2000k",
    ...
    mp4_path, webm_path = createSRMedia(mov_path)
    print('****', mp4_path, os.path.exists(mp4_path))
    print('****', webm_path, os.path.exists(webm_path))

    thumbnail_path = createThumbnails(mov_path)
    print('****', thumbnail_path, os.path.exists(thumbnail_path))
...
# PublishedFile = sg.sg.find('PublishedFile', [['id','is', 38]] ,sg.get_fields('PublishedFile') )
# result = sg.sg.upload(entity_type="PublishedFile", entity_id=38, path=r'V:\home\svilla\temp\sandbox\shot_1\Light\version_1\sh_001.mov', field_name='path', display_name=None, tag_list=None)
# print(PublishedFile)
# exit(-1)

# sg.sg.update("PublishedFile", 43, data={'version': {'type':'Version', 'id':7010}})
#
...
'''
project = sg.get_project('test_no_templates')
fields = sg.get_fields('Version')
filters = []
filters.append(['project', 'is',project ])
versions = sg.sg.find('Version', filters ,fields )
print(versions)
'''
'''
...
folder_to_scan = r'V:/home/svilla/temp/sandbox/shot_1/Light/version_5'
images_only = False
movies_only = True
geometry_only = False
content = 'model'
if True:
    ...
    filters = []
    filters.append(['project','is',project])
    filters.append(['code','is','asset_n1'])
    asset = sg.sg.find_one('Asset',filters,sg.get_fields('Asset'))

    #task
    filters = []
    filters.append(['project','is',project])
    filters.append(['entity','is',asset])
    filters.append(['content','is',content])
    task = sg.sg.find_one('Task',filters, sg.get_fields('Task'))

    #versions
    filters = []
    filters.append(['project', 'is',project ])
    #filters.append(['sg_task', 'is',task ])
    filters.append(['id', 'is',7030 ])

    versions = sg.sg.find('Version', filters ,sg.get_fields('Version') )
    print(versions)

    #version(project, asset, task, folder_to_scan)
...
if images:
    for image in images:
        data['sg_path_to_frames'] = os.path.dirname(image)
        version = sg.sg.create('Version', data)
        sg.sg.upload_thumbnail("Version", version['id'],image) 
'''
...
data = { 'project': project,
         ...
         'sg_path_to_frames': 'V:/home/svilla/temp/sandbox/shot_1/Light/version_1/sh_001.mov',
         'sg_path_to_movie':None,
         'sg_path_to_geometry': None,
         'sg_status_list': 'rev',
         'entity': {'type': 'Asset', 'id': asset['id']},
...
#if image?
sg.sg.upload_thumbnail("Version", version_id, "V:/home/svilla/temp/sandbox/shot_1/Light/version_1/sh_001.0110.png") #if image?

#if movie?
#sg.sg.upload_filmstrip_thumbnail("Version", version_id,'V:/home/svilla/temp/sandbox/shot_1/Light/version_1/sh_001.mov')
sg.sg.upload("Version", version_id,'V:/home/svilla/temp/sandbox/shot_1/Light/version_1/sh_001.mov')
'''

'''
from shotgrid import createVersion
createVersion.create_version(sg=sg.sg,project=project, data=data,  path_to_folder='V:/home/svilla/temp/sandbox/shot_1/Light/version_1')
'''