...
def transcodeVersion(sg, version, formats):
    ...
    if "thumbnails" in formats:
        ...
        for published_file in published_files:
            (thumbnail, filmstrip) = createFormatThumbnails(published_file, version.get('frame_range'))
            if thumbnail:
                print("Uploading thumbnail to Published File %s" % published_file.get('id'))
                sg.upload_thumbnail('PublishedFile', published_file.get('id'), thumbnail)
    ...
    if mov_path_published_file:
        ...
        updateVersion(sg, version.get('id'), None, None, mp4, webm, frame=version.get('sg_path_to_frames'))
...
def findDuration(file_path, first_frame):
    ...
    in_2 = ["cut", "-d", " ", "-f", "4"]
    in_3 = ["sed", "s/,//"]
    print(" | ".join([" ".join(duration_cmd), " ".join(in_1), " ".join(in_2), " ".join(in_3)]))
...
def create_version(sg, path_to_folder, data, project):
    """
    ...
    for imageType in imageTypes:
        filefound = findfile(os_walk_cache(path_to_folder), "*."+imageType)
        images.extend(filefound)
    for movieType in movieTypes:
        filefound = findfile(os_walk_cache(path_to_folder), "*." + movieType)
        movies.extend(filefound)

    '''baseFolder, subFolders = os_walk_cache(path_to_folder).next()
    ...
    for image in images:
        ...
        #data['code'] = imageBasenameWithoutExt,
        data['sg_path_to_frames'] = image
    ...
    for movie in movies:
        ...
        #data['code'] = movieBasenameWithoutExt
        data['sg_path_to_frames'] = movie
        version = sg.create("Version", data)
